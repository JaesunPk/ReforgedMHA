...
public class ZeroGravity extends QuirkCastManager {
    ...
    public boolean CastAbility1(Player p) {
        loc = RaycastUtils.StartRaycast(p,4,1);
        ArrayList<Entity> list;
        list = (ArrayList<Entity>) RaycastUtils.cleanTargetList(p, (List<Entity>) loc.getNearbyEntities(1,1,1),false);
        if(!list.isEmpty())
    ...
    public boolean CastAbility2(Player p) {
        ...
        for(Entity e : playerdata.get(p.getUniqueId()).getQuirkTaggedEntities()){if(e instanceof LivingEntity){ ((LivingEntity) e).addPotionEffect(new PotionEffect(PotionEffectType.LEVITATION,playerdata.get(p.getUniqueId()).getQuirk().getAbility2Effect(),8,true,true));} }
